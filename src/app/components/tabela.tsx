...
export default function Tabela(props: TabelaProps) {
    ...
    return (
      <td className="flex justify-center">
        ...
        {props.clienteExcluido ? (
          <button onClick={() => props.clienteExcluido?.(cliente)} className="flex justify-center items-center text-red-600 rounded-full p-2 m-1 hover:bg-purple-50">{iconeTrash}</button>
        ) : false}
      </td>
    )
  }
  ...
  return (
    <table className="w-full rounded-xl overflow-hidden">
      <thead className="bg-gradient-to-r from-purple-500 to bg-purple-800 text-gray-100">
        {renderizarCabecalho()}
      </thead>
      <tbody>
        {renderizarDados()}
